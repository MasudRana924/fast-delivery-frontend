...
export const getProduct = (keyword = "", category,color,brand) => async (dispatch) => {
  try {
    dispatch({ type: ALL_PRODUCT_REQUEST });
    let link = `/api/v1/products?keyword=${keyword}`;
    // const { data } = await axios.get('/api/v1/products');//productsRoutes
    ...
    if (color) {
      ...
      link = `/api/v1/products?keyword=${keyword}&color=${color}`;
    }
    if (brand) {

      link = `/api/v1/products?keyword=${keyword}&brand=${brand}`;
    }
